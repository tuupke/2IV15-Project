...
% TODO: Add images
To simulate 2-dimensional cloth, we implemented a function that generates a square grid of particles, interconnected by springs in the x and y-directions. Optionally, diagonal springs can be added. The two upper particles are fixed in the upper corners of the screen. A grid without diagonal springs behaves almost like cloth, with exception of shear forces, which make the grid collapse. This can be resolved by adding diagonal springs, which make the behavior of the cloth more natural. One downside of this simple model is that it can get 'stuck' in a local minimum. This could be mitigated by adding connectivity between a particle with particles more at distance, outside of the direct neighbors.
